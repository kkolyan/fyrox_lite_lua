...
#[no_mangle]
pub extern "C" fn fyrox_lite_lite_input_Input_is_mouse_button_pressed(button: i32) -> NativeBool {
    let button = button.into();
...
#[no_mangle]
pub extern "C" fn fyrox_lite_lite_input_Input_is_key_pressed(key: NativeKeyCode) -> NativeBool {
    let key = key.into();
...
pub extern "C" fn fyrox_lite_lite_physics_LitePhysics_cast_ray(
    opts: *mut NativeRayCastOptions,
) -> NativeIntersection_slice {
    let opts = unsafe { *opts }.into();
...
pub extern "C" fn fyrox_lite_lite_physics_LiteRigidBody_apply_force(
    ...
    force: *mut NativeVector3,
) -> () {
    let force = unsafe { *force }.into();
